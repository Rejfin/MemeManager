...
const AuthPage = (props: { isRegisterPage: boolean }) => {
  ...
  const isPasswordFieldValid = (): boolean => {
    if (password.length === 0) {
      ...
      (password.length < 6 || !/[0-9]+/.test(password) || !/[!@#$%^&*()_+\-=[\]{};':"\\|,.<>/?]+/.test(password))
    ) {
  ...
  const handleSubmit = (event: React.SyntheticEvent) => {
    ...
    if (props.isRegisterPage) {
      if (isLoginFieldValid() && isPasswordFieldValid() && isRepeatedPasswordFieldValid()) {
  ...
  return (
    <div className='flex flex-col items-center justify-center h-full w-full'>
      <div className='flex flex-col items-center w-full h-fit md:w-4/5 md:h-fit md:max-w-3xl bg-backgroundSurface dark:bg-backgroundSurface-dark rounded-md p-4'>
        <div className='flex items-center justify-center'>
          ...
          <div className='flex flex-col items-start justify-center pl-3'>
            ...
            <h2 className='text-textColor dark:text-textColor-dark cursor-default'>{t('appDescription')}</h2>
          </div>
        </div>
        <form className='flex flex-col gap-y-4 items-center justify-center pt-5 w-96 max-w-sm'>
          ...
          {props.isRegisterPage === true ? (
            <>
              ...
              />
              <button onClick={handleSubmit} className='bg-primary-500 text-textColor-dark rounded-lg py-3 w-60'>
                {t('auth.register') || ''}
              </button>
            </>
          ) : (
      ...
      <div className='pt-16'>
        {props.isRegisterPage === true ? (
          <>
            <h3 className='inline text-textColor dark:text-textColor-dark'>{t('auth.alreadyHaveAcc')}</h3>
            <Link to='/login' className='inline text-primary-600 pl-2 cursor-pointer'>
              {t('auth.logIn')}
            </Link>
          </>
        ) : (
          <>
            <h3 className='inline text-textColor dark:text-textColor-dark'>{t('auth.dontHaveAcc')}</h3>
            <Link to='/register' className='inline text-primary-600 pl-2 cursor-pointer'>
